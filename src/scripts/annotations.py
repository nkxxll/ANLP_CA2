...
def update_df_review_labels(df: pd.DataFrame, labels: dict, mode: str = "dummy") -> pd.DataFrame:
...
def get_one_hot_labels_df_(labels: dict) -> pd.DataFrame:
    ...
    # get unique labels
    all_labels = set([l for label in labels.values() for l in label]) # flatten list of all unique labels
    print(f"mapping {len(all_labels)} unique labels: {', '.join(all_labels)}")
    
    # for each label, create a hot one encoded column
    dummy_cols = pd.DataFrame({label: [1 if label in labels[key] else 0 for key in labels] for label in all_labels}, index=labels.keys())
    return dummy_cols